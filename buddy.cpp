...
#define BITMAP_XOR(FIRST, ORDER) bitmaps[ORDER][BIT_POS(FIRST, ORDER)] = bitmaps[order][BIT_POS(FIRST, ORDER)] ^ 1
...
class BuddyAllocator {
    ...
    public:
        BuddyAllocator(int first, int last, int df_order);
...
BuddyAllocator::BuddyAllocator(int first, int last, int df_order = 10)
{
    base = first;
    total_size = last - first + 1;
    int order = int(log2(total_size));
    max_order = min(order, df_order);
...
void BuddyAllocator::Merge(int order, int first)
{
    ...
    for (hit = 0, it = area.begin(); it != area.end() &&  hit < 2;) {
...
int main(int argc, char *argv[])
{
    ...
    while (count--) {
        ...
        if (action) {
            ...
            if (id == INT_MAX)
                break;
            cout << "Allocated size " << sz << " and first id is " << id << "." << endl;
            buddy.Dump();
            test_id node = { id, sz };
            ids.push_front(node);
        }
